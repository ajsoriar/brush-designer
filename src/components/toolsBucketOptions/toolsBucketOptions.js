(function(global) {
    ...
    function ToolsBucketOptionsComponent(options) {
        ...
        var modeInputs = {};
        var componentId = config.id || ("tools-bucket-options-" + Date.now());
        var component;

        element.id = componentId;
        element.className = "tools-bucket-options";
        legend.textContent = "Bucket";

        grid.className = "tools-bucket-options-grid";

        toleranceLabel.className = "tools-bucket-options-label";
        toleranceLabel.textContent = "Tolerance";

        toleranceRange.className = "tools-bucket-options-range";
        toleranceRange.type = "range";
        toleranceRange.min = "0";
        toleranceRange.max = "100";
        toleranceRange.step = "1";

        toleranceNumber.className = "tools-bucket-options-number";
        ...
        antiAliasLabel.className = "tools-bucket-options-check";
        ...
        contiguousLabel.className = "tools-bucket-options-check";
        ...
        sampleAllLayersLabel.className = "tools-bucket-options-check";
        ...
        modeGroup.className = "tools-bucket-options-modes";
        ...
        setOptionsOnInputs(getCurrentBucketOptions());
        ...
        antiAliasInput.addEventListener("change", function() {
            if (global.PaintTools && global.PaintTools.setBucketAntiAlias) {
                global.PaintTools.setBucketAntiAlias(antiAliasInput.checked);
            }
        });

        contiguousInput.addEventListener("change", function() {
            if (global.PaintTools && global.PaintTools.setBucketContiguous) {
                global.PaintTools.setBucketContiguous(contiguousInput.checked);
            }
        });

        sampleAllLayersInput.addEventListener("change", function() {
            if (global.PaintTools && global.PaintTools.setBucketSampleAllLayers) {
                global.PaintTools.setBucketSampleAllLayers(sampleAllLayersInput.checked);
            }
        });

        bindModeRadios(modeInputs, function(mode) {
            if (global.PaintTools && global.PaintTools.setBucketMode) {
                global.PaintTools.setBucketMode(mode);
            }
        });

        global.addEventListener("paint-bucket-options-change", function(event) {
        ...
        function updateTolerance(value) {
            ...
            if (global.PaintTools && global.PaintTools.setBucketTolerance) {
                global.PaintTools.setBucketTolerance(value);
            }
        }
    ...
    function buildModeRadios(modeGroup, modeInputs, componentId) {
        ...
        for (i = 0; i < MODES.length; i++) {
            ...
            label.className = "tools-bucket-options-mode";
    ...
    function getCurrentBucketOptions() {
        ...
        if (global.PaintTools.getBucketOptions) {
            return global.PaintTools.getBucketOptions();
        }
    ...
    function getContainer(containerId) {
        ...
        if (!container) {
            throw new Error("ToolsBucketOptions container not found: " + containerId);
        }

        return container;
    }

    global.ToolsBucketOptionsComponent = ToolsBucketOptionsComponent;
    global.toolsBucketOptions = ToolsBucketOptionsComponent;
